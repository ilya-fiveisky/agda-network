...
main = run $ do
  ...
  putStrLn $ show r
  r ← curl $ data′ (a (str "")) ∷ ？ "--url" ∷ ？ "https://www.example.com/guest.cgi" ∷ []
  putStrLn $ show r 
  r ← curl $ d (str "name=xxx") ∷ ？ "--url" ∷ ？ "https://www.example.com/guest.cgi" ∷ []
  putStrLn $ show r
  -- For some weird reason all full --data options don't work (though --data with empty data and -d work).
  -- Nevertheless directly in terminal copied curl cmdLine works.
  r ← curl $ data′ (a (str "xxx")) ∷ ？ "--url" ∷ ？ "https://www.example.com/guest.cgi" ∷ []
  putStrLn $ show r 
